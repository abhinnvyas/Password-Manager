    ...
    def loginFrame(self):
        ...
        def login():
            ...
            if len(result) == 1:
                ...
                msg.showerror("Login Status", "Login Failed!\nIncorrect Username or Password.")
                usrname.set("")
                password.set("")
                frame1.tkraise()
        ...
        #Heading
        heading = tk.Label(emptyFrame,text="Login", bg="white", font=("Times New Roman",14))
        ...
        login_button = tk.Button(frame1, text="Login", command= login, bg='blue',fg="white",relief=tk.FLAT)
        login_button.place(relx=0.5, rely=0.6,anchor=tk.CENTER, width=200)

        login_button.bind("<Return>",lambda event: login())

        signupSwitch = tk.Button(self.root, text="Create an Account", bg="white",fg="blue",relief=tk.FLAT, command=self.SignUpFrame)
        signupSwitch.place(relx=0.6,rely=0.51,anchor=tk.CENTER,width=200,height=17)
                ...
                if passw1 == passw2:
                    if self.validatePassword(passw1):
                        ...
                        try:
                            ...
                            msg.showerror("Error", f"{self.USERNAME} already Exists")
                            usrname.set("")
                            password.set("")
                            rePassword.set("")
                            self.USERNAME = ""
...
4. Should be between 6 to 20 characters long.")
        ...
        #Heading
        heading = tk.Label(emptyFrame,text="Register", bg="white", font=("Times New Roman",14))
        heading.pack(fill=tk.X, pady=60)
        f = font.Font(heading, heading.cget("font"))
        f.configure(underline=True)
        heading.configure(font=f)
        ...
        #username entry
        tk.Label(signupFrame, text="Username ", bg="white").grid(row=0, column=0)
        usrname = tk.StringVar()
        usrname.set("")
        ...
        #password entry
        tk.Label(signupFrame, text="Password ", bg="white").grid(row=1, column=0)
        password = tk.StringVar()
        password.set("")
        ...
        #repassword entry
        tk.Label(signupFrame, text="Confirm Password ", bg="white").grid(row=2, column=0)
        rePassword = tk.StringVar()
        password.set("")
        re_passw_entry = tk.Entry(signupFrame, textvariable=rePassword,show="*")
        re_passw_entry.grid(row=2, column=1)

        signup_button = tk.Button(signupFrame, text="Create Account", command= signup, bg='blue',fg="white",relief=tk.FLAT)
        signup_button.place(relx=0.5, rely=0.8,anchor=tk.CENTER, width=200)

        signup_button.bind("<Return>",lambda event: signup())

        signupSwitch = tk.Button(self.root, text="Login", bg="white",fg="blue",relief=tk.FLAT, command=self.loginFrame)
    ...
    def FunctionFrame(self):
        ...
        def functions():
                ...
                try:
                    ...
                    if selection != 0:
                        ...
                        try:
                            ...
                            msg.showinfo("Info","Element Deleted Successfully.")
                            listbox.delete(selection)
                    else:
                        msg.showwarning("Warning","Can't Delete this Element.")
            ...
            def deleteAccount():
                ask = msg.askyesnocancel("Delete","Do you want to Delete this Account?")
                if ask:
                    try:
            ...
            def updateListBox():
                ...
                self.cursor.execute(self.QUERY)
                result = [u"⚫"+f"{i[0]} | {i[1]} | {i[2]}" for i in self.cursor.fetchall()]
            ...
            tk.Label(functions_frame, text="Add New",font=("times new roman",12), bg="black", fg="white", width=24).grid(padx=2,pady=7,row=0, columnspan=2)
            ...
            tk.Label(functions_frame, text="Platform", bg="white").grid(row=1,column=0)
            platform_entry = tk.Entry(functions_frame, textvariable=platform, relief=tk.SUNKEN)
            platform_entry.grid(row=1, column=1)
            platform_entry.focus()

            tk.Label(functions_frame, text="Username", bg="white").grid(row=2,column=0)
            tk.Entry(functions_frame, textvariable=usr, relief=tk.SUNKEN).grid(row=2,column=1)

            tk.Label(functions_frame, text="Password", bg="white").grid(row=3,column=0)
            tk.Entry(functions_frame, textvariable=passw, relief=tk.SUNKEN).grid(row=3,column=1)

            add = tk.Button(functions_frame, text="Add", bg="Black", fg="white", command=addElement, width=30)
            add.grid(pady=1,row=4, columnspan=2)

            add.bind("<Return>",lambda event: addElement())

            #Code for Deleting the Current Entry
            del_selected = tk.Button(functions_frame,bg="white",fg="red", text="Delete Selected", width=30, command=deleteSelected, relief=tk.RIDGE)
            del_selected.grid(padx=15,row=0, column=2,columnspan=2)

            del_selected.bind("<Return>",lambda event: deleteSelected())

            #Code for Log Out
            logout = tk.Button(functions_frame, text="Logout",bg="white",fg="red",width=30, bd=2, relief=tk.RIDGE, command=logout)
            logout.grid(row=1, column=2,columnspan=2)

            logout.bind("<Return>",lambda event: logout())

            #Code for the Exit
            exitButton = tk.Button(functions_frame, text="Quit",bg="white",fg="red",width=30, bd=2, relief=tk.RIDGE, command=Exit)
            exitButton.grid(row=2, column=2,columnspan=2, pady=3)

            exitButton.bind("<Return>",lambda event: Exit())

            deleteAccButton = tk.Button(functions_frame, text="Delete Account",bg="white",fg="red",width=30, bd=2, relief=tk.RIDGE, command=deleteAccount)
        ...
        listbox = tk.Listbox(lis_box_frame, selectmode=tk.BROWSE ,width=300, relief=tk.RIDGE, font=("times new roman",12), bg="white", yscrollcommand=scrollBar.set)
